xml.rss( :version => '2.0', 'xmlns:atom' => 'http://www.w3.org/2005/Atom' ) do
  xml.channel do
    xml.title( "Work packages for the #{@organisation_accountable_to_parliament.label} currently before Parliament" )
    xml.description( "Updates whenever a new work package is added." )
	xml.link( organisation_accountable_to_parliament_work_package_current_list_url )
    ...
    xml.tag!( 'atom:link', { :href => organisation_accountable_to_parliament_work_package_current_list_url( :format => 'rss' ), :rel => 'self', :type => 'application/rss+xml' } )

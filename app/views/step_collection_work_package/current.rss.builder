xml.rss( :version => '2.0', 'xmlns:atom' => 'http://www.w3.org/2005/Atom' ) do
  xml.channel do
    xml.title( "Work packages before Parliament actualising one or more steps in the '#{@step_collection.label}' step collection." )
    xml.description( "Updates whenever a work package actualises a step in the '#{@step_collection.label}' step collection." )
	xml.link( step_collection_work_package_current_list_url )
    ...
    xml.tag!( 'atom:link', { :href => step_collection_work_package_current_list_url( :format => 'rss' ), :rel => 'self', :type => 'application/rss+xml' } )

xml.rss( :version => '2.0', 'xmlns:atom' => 'http://www.w3.org/2005/Atom' ) do
	xml.channel do
	    ...
	    xml.language( 'en-uk' )
	    xml.pubDate( @work_package_business_items.first.first.first.date.rfc822 ) unless @work_package_business_items.empty? or @work_package_business_items.first.first.first.date.nil?
	    xml.tag!( 'atom:link', { :href => work_package_business_item_list_url( :format => 'rss' ), :rel => 'self', :type => 'application/rss+xml' } )
		@work_package_business_items.each do |date|
			xml << render(:partial => 'business_item', :collection => date ) unless date.empty?
		end
	end
end
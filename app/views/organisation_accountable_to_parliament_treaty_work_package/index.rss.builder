xml.rss( :version => '2.0', 'xmlns:atom' => 'http://www.w3.org/2005/Atom' ) do
  xml.channel do
    xml.title( "Work packages for treaties led by the #{@organisation_accountable_to_parliament.label}" )
    xml.description( "Updates whenever a new work package is added." )
	xml.link( organisation_accountable_to_parliament_treaty_work_package_list_url )
    ...
    xml.tag!( 'atom:link', { :href => organisation_accountable_to_parliament_treaty_work_package_list_url( :format => 'rss' ), :rel => 'self', :type => 'application/rss+xml' } )

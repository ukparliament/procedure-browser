xml.rss( :version => '2.0', 'xmlns:atom' => 'http://www.w3.org/2005/Atom' ) do
  xml.channel do
    xml.title( "Work packages subject to the '#{@procedure.label}' procedure currently before Parliament." )
    xml.description( "Updates whenever a new work package is created." )
	xml.link( procedure_work_package_current_list_url )
    ...
    xml.tag!( 'atom:link', { :href => procedure_work_package_current_list_url( :format => 'rss' ), :rel => 'self', :type => 'application/rss+xml' } )

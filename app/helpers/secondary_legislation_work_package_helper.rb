module SecondaryLegislationWorkPackageHelper

  def secondary_legislation_work_package_results_per_page_link( number, results_per_page )
    work_package_results_per_page_link = ''
    if number == results_per_page
      work_package_results_per_page_link += number.to_s
    else
      link_to( number.to_s, work_package_secondary_legislation_list_url( 'results-per-page' => number ) )
    end
  end
  
  # A method to generate the previous page link, if any.
  def secondary_legislation_work_package_list_pagination_previous_link( page, results_per_page )
    previous_link = ''
    ...
    if page == 2
      ...
      if results_per_page == $DEFAULT_RESULTS_PER_PAGE
        ...
        previous_link += work_package_secondary_legislation_list_url
      ...
      # Otherwise, if results per page is not set to the default ... 
        ...
        previous_link += work_package_secondary_legislation_list_url( 'results-per-page' => results_per_page )
      end
    ...
    # Otherwise, if this is the third page or above ...
      ...
      if results_per_page == $DEFAULT_RESULTS_PER_PAGE
        ...
        previous_link += work_package_secondary_legislation_list_url( 'page' => page - 1 )
        
      # Otherwise, if results per page is not set to the default ...
      else
      
        # ... we link to the preceding page of the list with the results per page parameter.
        previous_link += work_package_secondary_legislation_list_url( 'page' => page - 1, 'results-per-page' => results_per_page )
      end
    end
  ...
  def secondary_legislation_work_package_list_pagination_next_link( page, results_per_page, result_count )
    next_link = ''
    ...
    if pagination_has_subsequent_page( page, results_per_page, result_count )
      ...
      if results_per_page == $DEFAULT_RESULTS_PER_PAGE
        ...
        next_link += work_package_secondary_legislation_list_url( 'page' => page + 1 )
        
      # Otherwise, if results per page is not set to the default 
      else
      
        # ... we link to the next page with the result per page parameter.
        next_link += work_package_secondary_legislation_list_url( 'page' => page + 1, 'results-per-page' => results_per_page )
      end
    end
    next_link
  end

module Sparql::Get::OrganisationAccountableToParliamentWorkPackages
  
  def get_organisation_accountable_to_parliament_work_packages( organisation_accountable_to_parliament_id, page_number, results_per_page )
  
    # We calculate the result offset.
    result_offset = ( ( page_number - 1) * results_per_page )
    
    # We get the organisation accountable to parliament work packages query.
    request_body = organisation_accountable_to_parliament_work_packages_query( organisation_accountable_to_parliament_id, results_per_page, result_offset )

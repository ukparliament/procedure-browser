module Sparql::Get::OrganisationsAccountableToParliamentCurrent

  # A method to get an array of current organisations accountable to Parliament on this page with this many results per page.
  def get_organisations_accountable_to_parliament_current( page_number, results_per_page )
  
    # We calculate the result offset.
    result_offset = ( ( page_number - 1) * results_per_page )

    # We get the current organisations accountable to Parliament query passing parameters for the number of results per page and the result offset.
    request_body = organisations_accountable_to_parliament_current_query( results_per_page, result_offset )

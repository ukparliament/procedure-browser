module Sparql::Get::EnablingLegislationWorkPackages
  
  def get_enabling_legislation_work_packages( enabling_thing_id, page_number, results_per_page )
  
    # We calculate the result offset.
    result_offset = ( ( page_number - 1) * results_per_page )
    
    # We get the enabling legislation work packages query.
    request_body = enabling_legislation_work_packages_query( enabling_thing_id, results_per_page, result_offset )

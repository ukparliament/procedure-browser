module Sparql::Get::WorkPackageableThingsCurrent

  # A method to get an array of current work packageable things on this page with this many results per page.
  def get_work_packageable_things_current( page_number, results_per_page )
  
    # We calculate the result offset.
    result_offset = ( ( page_number - 1) * results_per_page )

    # We get the current work packageable things query passing parameters for the number of results per page and the result offset.
    request_body = work_packageable_things_current_query( results_per_page, result_offset )

module Sparql::Get::WorkPackagesCurrent

  # A method to get an array of current work packages on this page with this many results per page.
  def get_work_packages_current( page_number, results_per_page )
  
    # We calculate the result offset.
    result_offset = ( ( page_number - 1) * results_per_page )

    # We get the current work packages query passing parameters for the number of results per page and the result offset.
    request_body = work_packages_current_query( results_per_page, result_offset )

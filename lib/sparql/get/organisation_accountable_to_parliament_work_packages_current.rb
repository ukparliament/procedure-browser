module Sparql::Get::OrganisationAccountableToParliamentWorkPackagesCurrent
  
  def get_organisation_accountable_to_parliament_current_work_packages( organisation_accountable_to_parliament_id, page_number, results_per_page )
  
    # We calculate the result offset.
    result_offset = ( ( page_number - 1) * results_per_page )
    
    # We get the organisation accountable to parliament current work packages query.
    request_body = organisation_accountable_to_parliament_work_packages_current_query( organisation_accountable_to_parliament_id, results_per_page, result_offset )
    ...
    csv.each do |row|
      ...
      work_package.work_packageable_thing_identifer = row['LaidThing']
      work_package.work_packageable_thing_label = row['LaidThingName']

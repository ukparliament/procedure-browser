module Sparql::Get::SecondaryLegislationWorkPackages

  # A method to get an array of all primary legislation work packages on this page with this many results per page.
  def get_secondary_legislation_work_packages( page_number, results_per_page )
  
    # We calculate the result offset.
    result_offset = ( ( page_number - 1) * results_per_page )

    # We get the secondary legislation work packages query passing parameters for the number of results per page and the result offset.
    request_body = secondary_legislation_work_packages_query( results_per_page, result_offset )

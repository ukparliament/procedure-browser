module Sparql::Get::TreatyWorkPackages

  # A method to get an array of all treaty work packages on this page with this many results per page.
  def get_treaty_work_packages( page_number, results_per_page )
  
    # We calculate the result offset.
    result_offset = ( ( page_number - 1) * results_per_page )

    # We get the treaty work packages query passing parameters for the number of results per page and the result offset.
    request_body = treaty_work_packages_query( results_per_page, result_offset )

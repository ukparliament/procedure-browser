module Sparql::Get::EnablingLegislationWorkPackagesCurrent
  
  def get_enabling_legislation_work_packages_current( enabling_thing_id, page_number, results_per_page )
  
    # We calculate the result offset.
    result_offset = ( ( page_number - 1) * results_per_page )
    
    # We get the enabling legislation current work packages query.
    request_body = enabling_legislation_work_packages_current_query( enabling_thing_id, results_per_page, result_offset )

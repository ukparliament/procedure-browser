...
SELECT ?legislature ?legislatureName ?step ?stepName ?stepType ?stepTypeName (COUNT(?businessItem) AS ?businessItemCount) WHERE { 
	...
	# We specify that the legislature's procedure steps may, or may not, have been actualised in a business item. 
	# This is then used in the SELECT line to work out a count of the number of business items that the procedure step has been actualised in.
	OPTIONAL {
		?step :procedureStepHasBusinessItem ?businessItem 
	}  
} 
# We use GROUP BY because COUNT(?businessItem) in the SELECT line is an aggregate. 
# We want the count calculated separately for each procedure step (and its associated legislature and step type details).

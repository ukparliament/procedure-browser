...
WHERE {
  ...
  # A business item or items may actualise two different business steps in the same work package on the same day.
  # As we capture dates rather than times of business items it was not possible to order business items in a way that made 'procedural sense', for example: we may have shown 'Procedure concluded' before 'Instrument approved'. 
  # procedureStepHasStepDisplayDepthInProcedure records the display depth of a business step forming part of the procedure, associating a display depth number with each step. For example: the 'Instrument created' business step in the draft affirmative procedure has a display depth of 1.0, whereas the 'Laid before the House of Commons' business step in the same procedure has a display depth of 3.1. These numbers are used to order business items in a work package sharing the same date.
  OPTIONAL {
  ...
  # We look for the work package's procedure and its work packaged thing. 
  # A work packaged thing is a paper made available to Parliament, for example a statutory instrument.
